package store.mybooks.resource.returnrule.controller;
...
import store.mybooks.resource.returnrule.service.ReturnRuleService;
...
class ReturnRuleRestControllerUnitTest {
    ...
    void givenReturnRuleCreateRequest_whenCreateReturnRule_thenReturnReturnRuleCreateResponse() throws Exception {
        ...
        ReturnRuleCreateRequest request = new ReturnRuleCreateRequest();
        ReflectionTestUtils.setField(request, "returnName", "test");
        ReflectionTestUtils.setField(request, "deliveryFee", 100);
        ReflectionTestUtils.setField(request, "term", 10);
    ...
    void givenReturnRuleNameBlank_when_thenReturnHttpStatusCodeIsNotFound() throws Exception {
        ObjectMapper mapper = new ObjectMapper();
        ReturnRuleCreateRequest request = new ReturnRuleCreateRequest();
        ReflectionTestUtils.setField(request, "returnName", "");
        ReflectionTestUtils.setField(request, "deliveryFee", 100);
        ReflectionTestUtils.setField(request, "term", 10);
        mockMvc.perform(post("/api/return-rules")
                        .content(mapper.writeValueAsString(request))
    ...
    void givenReturnRuleDeliveryFee_when_thenReturnHttpStatusCodeIsNotFound() throws Exception {
        ...
        ReturnRuleCreateRequest request = new ReturnRuleCreateRequest();
        ReflectionTestUtils.setField(request, "returnName", "test");
        ReflectionTestUtils.setField(request, "deliveryFee", -1);
        ReflectionTestUtils.setField(request, "term", 10);
        mockMvc.perform(post("/api/return-rules")
                        .content(mapper.writeValueAsString(request))
                        .contentType(MediaType.APPLICATION_JSON))
    ...
    void givenReturnRuleTerm_when_thenReturnHttpStatusCodeIsNotFound() throws Exception {
        ...
        ReturnRuleCreateRequest request = new ReturnRuleCreateRequest();
        ReflectionTestUtils.setField(request, "returnName", "test");
        ReflectionTestUtils.setField(request, "deliveryFee", 1000);
        ReflectionTestUtils.setField(request, "term", 366);
        mockMvc.perform(post("/api/return-rules")
                        .content(mapper.writeValueAsString(request))
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isBadRequest())
