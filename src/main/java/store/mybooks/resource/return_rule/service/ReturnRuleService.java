...
public class ReturnRuleService {
    ...
    public ReturnRuleCreateResponse createReturnRule(ReturnRuleCreateRequest request) {
        ...
        ReturnRuleName returnRuleName =
                returnRuleNameRepository.findById(request.getReturnName()).orElseThrow(() -> new ReturnRuleNameAlreadyExistException("이미 존재하는 반품 정책 명입니다."));

        ReturnRule returnRule = new ReturnRule(1, request.getDeliveryFee(), request.getTerm(), true, LocalDate.now(), returnRuleName);
